...
class Wav2Vec2ConformerNoAsrSERModel(L.LightningModule):
    LOCAL = "local"
    ...
    def __init__(
        ...
        asr_model_name="facebook/wav2vec2-conformer-rope-large-960h-ft",
        ...
        self.asr_base = Wav2Vec2ConformerModel.from_pretrained(asr_model_name)
        for param in self.embedding.parameters():
            param.requires_grad = False
        for param in self.asr_base.parameters():
            param.requires_grad = False
    ...
    def forward(self, x, mask, embedding=None):
        if embedding is None:
            x_emb = self.embedding(x, output_hidden_states=True)
            x_asr = self.asr_base(x, output_hidden_states=True)

            x = Wav2Vec2BaseModelOutput(
                hidden_states=tuple(
                    emb_layer - asr_layer
                    for emb_layer, asr_layer in zip(
                        x_emb.hidden_states, x_asr.hidden_states
                    )
                ),
                extract_features=x_emb.extract_features - x_asr.extract_features,
            )
        else:
            x = embedding
